...
public class EventScheduleScanner implements Service, Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            cache.query(new ScanQuery<Long, Bucket>((k, v) ->
                ...
                final int numShards = (int) e.getValue().getCount() / shardSize + 1;
                L.debug(format("%d, offset: %d, numShards: %d, count: %d, shardSize: %d", currentOffset, e.getKey(), numShards, e.getValue().getCount(), shardSize));
    ...
    public static class JobCountSetter implements EntryProcessor<Long, Bucket, Object> {
        @Override
