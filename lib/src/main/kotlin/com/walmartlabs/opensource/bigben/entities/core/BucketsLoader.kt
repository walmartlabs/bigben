...
import com.walmartlabs.opensource.bigben.entities.EventStatus.EMPTY
import com.walmartlabs.opensource.bigben.entities.core.BucketManager.Companion.scheduler
            ...
            (1..fetchSize).forEach {
                ...
                if (bucketIndex <= lookbackRange) {
                    ...
                    if (!predicate.test(bId)) {
                        ...
                        taskExecutor.async("bucket-load:" + bId, lookbackRange - bucketIndex + 1, bucketWidth, 1) { fetch<Bucket>(bId) }
                                .done({ l.error("error in loading bucket {}, system is giving up", bId, getRootCause(it)) }) {
                                    if (l.isDebugEnabled)
                                        l.info("bucket {} loaded successfully", bId)
                                    consumer(it ?: Bucket(bId, EMPTY, 0L))
                                }
                    } else {
